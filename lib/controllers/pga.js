...
exports.tournamentSetup = function(req, res) {
  ...
  //*** 1. set expired date to minus 1 day
  var ageLimit = new Date();
  ageLimit.setDate(ageLimit.getDate() - 1);


  //*** 2. Search db for setup record since expired date
  Tournament.findOne({modified: {$gte: ageLimit}},
    function(err, setupRecord) {
      ...
      if(!setupRecord){
        console.log('no setup record found');
      ...
      if(!playerRecord){
        ...
        request({uri: 'http://www.pgatour.com/data/r/current/scorecards/' + playerId + '.json', json: true},
          function(error, response, body) {
            ...
            if(error){return res.json(500, error);}
            console.log('player record retrieved');
            ...
            //check for player rounds in raw response
